            ...
            &mut controllers
        {
            match event {
                MovementAction::Move(direction) => {
                    // Calculate movement direction based on the player's current rotation (view angle)
                    let forward = transform.forward();
                    let right = transform.right();

                    // Project forward onto the XZ plane to ignore the Y component
                    let forward_xz = forward.reject_from_normalized(Vec3::Y).normalize_or_zero();
                    let movement_direction = forward_xz * direction.y + right * direction.x;

                    linear_velocity.0 += movement_direction.normalize_or_zero()
                        * movement_acceleration.0
                        * delta_time;
                }
                MovementAction::Jump => {
                    if is_grounded {
                        // Apply jump impulse, ensuring the vertical component is handled properly
                        linear_velocity.y = jump_impulse.0.max(linear_velocity.y);
                    }
                }
            }
        }
...
/// Updates the player's view direction based on mouse motion.
fn move_player(
    ...
    // Store pitch and yaw separately
    ...
    for motion in mouse_motion.read() {
        ...
        // Clamp pitch to avoid neck-breaking movement
        pitch = pitch.clamp(-PI / 2.0 + 0.1, PI / 2.0 - 0.1);

        // Apply the new yaw and clamped pitch to the transform's rotation
